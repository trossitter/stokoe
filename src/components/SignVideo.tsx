...
function Placeholder({ word, reason }: { word: string; reason: "missing" | "error" }) {
  return (
    <div className="aspect-video flex flex-col items-center justify-center gap-1 bg-slate-50 rounded-xl border border-slate-100">
      <svg
      ...
      <p className="text-xs text-slate-400 text-center leading-tight">
        ...
        <br />
        <span className="text-slate-300">for {word}</span>
      </p>
    </div>
  );
}
...
export function SignVideo({ item, hidden }: Props) {
  ...
  return (
    <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-3 flex flex-col gap-2">
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        {videoUrl && !error && (
          <button
            ...
            className="w-6 h-6 flex items-center justify-center rounded-md text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors"
